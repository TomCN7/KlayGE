...
namespace KlayGE
{
			...
			inv_timestamp_freq_(0)
	{
		...
		if (mod_d3d11_ != nullptr)
		{
			...
			DynamicD3D11On12CreateDevice_ = reinterpret_cast<D3D11On12CreateDeviceFunc>(::GetProcAddress(mod_d3d11_, "D3D11On12CreateDevice"));
		}
		...
		DynamicD3D11On12CreateDevice_ = ::D3D11On12CreateDevice;
		DynamicD3D12GetDebugInterface_ = ::D3D12GetDebugInterface;
#endif

		IDXGIFactory4* gi_factory;
		TIF(DynamicCreateDXGIFactory1_(IID_IDXGIFactory4, reinterpret_cast<void**>(&gi_factory)));
	...
	void D3D12RenderEngine::BeginFrame()
	{
		...
		{
			d3d_11_imm_ctx_->Begin(timestamp_disjoint_query_.get());
		}
	...
	void D3D12RenderEngine::EndFrame()
	{
		...
		if ((d3d_feature_level_ >= D3D_FEATURE_LEVEL_10_0) && Context::Instance().Config().perf_profiler)
		{
			d3d_11_imm_ctx_->End(timestamp_disjoint_query_.get());
		}
	}
	...
	void D3D12RenderEngine::UpdateGPUTimestampsFrequency()
	{
		if (d3d_feature_level_ >= D3D_FEATURE_LEVEL_10_0)
		{
			...
			while (S_OK != d3d_11_imm_ctx_->GetData(timestamp_disjoint_query_.get(), &disjoint, sizeof(disjoint), 0));
	...
	ID3D12CommandQueuePtr const & D3D12RenderEngine::D3D12CmdQueue() const
	{
	...
	{
		return d3d_11_device_;
	}

	// 获取D3D Device Context接口
	/////////////////////////////////////////////////////////////////////////////////
	ID3D11DeviceContextPtr const & D3D12RenderEngine::D3D11DeviceImmContext() const
	{
		return d3d_11_imm_ctx_;
	}
		...
		RenderSettings const & settings)
	{
		...
		if (d3d_feature_level_ >= D3D_FEATURE_LEVEL_10_0)
		{
			...
			d3d_11_device_->CreateQuery(&desc, &disjoint_query);
	...
	void D3D12RenderEngine::D3DDevice(ID3D12DevicePtr const & device_12, ID3D12CommandQueuePtr const & cmd_queue,
		ID3D11DevicePtr const & device_11, ID3D11DeviceContextPtr const & imm_ctx, D3D_FEATURE_LEVEL feature_level)
	{
		...
		d3d_12_cmd_queue_ = cmd_queue;

		ID3D11Device2* d3d_device_2;
		device_11->QueryInterface(IID_ID3D11Device2, reinterpret_cast<void**>(&d3d_device_2));
		if (d3d_device_2)
		{
			ID3D11DeviceContext2* d3d_11_imm_ctx_2;
			imm_ctx->QueryInterface(IID_ID3D11DeviceContext2, reinterpret_cast<void**>(&d3d_11_imm_ctx_2));
			if (d3d_11_imm_ctx_2)
			{
				d3d_11_device_ = MakeCOMPtr(d3d_device_2);
				d3d_11_imm_ctx_ = MakeCOMPtr(d3d_11_imm_ctx_2);
			}
			else
			{
		...
		Verify(!!d3d_11_device_);
		Verify(!!d3d_11_imm_ctx_);
	...
	void D3D12RenderEngine::ResetRenderStates()
	{
		...
		rasterizer_state_cache_ = checked_cast<D3D12RasterizerStateObject*>(cur_rs_obj_.get())->D3DRasterizerState();
		depth_stencil_state_cache_ = checked_cast<D3D12DepthStencilStateObject*>(cur_dss_obj_.get())->D3DDepthStencilState();
		stencil_ref_cache_ = 0;
		blend_state_cache_ = checked_cast<D3D12BlendStateObject*>(cur_bs_obj_.get())->D3DBlendState();
		blend_factor_cache_ = Color(1, 1, 1, 1);
		sample_mask_cache_ = 0xFFFFFFFF;

		d3d_11_imm_ctx_->RSSetState(rasterizer_state_cache_.get());
		d3d_11_imm_ctx_->OMSetDepthStencilState(depth_stencil_state_cache_.get(), stencil_ref_cache_);
		d3d_11_imm_ctx_->OMSetBlendState(blend_state_cache_.get(), &blend_factor_cache_.r(), sample_mask_cache_);

		topology_type_cache_ = RenderLayout::TT_PointList;
		d3d_11_imm_ctx_->IASetPrimitiveTopology(D3D12Mapping::Mapping(topology_type_cache_));

		input_layout_cache_.reset();
		d3d_11_imm_ctx_->IASetInputLayout(input_layout_cache_.get());
		...
		if (!vb_cache_.empty())
		{
			...
			d3d_11_imm_ctx_->IASetVertexBuffers(0, static_cast<UINT>(vb_cache_.size()),
		...
		ib_cache_.reset();
		d3d_11_imm_ctx_->IASetIndexBuffer(ib_cache_.get(), DXGI_FORMAT_R16_UINT, 0);
		...
		for (uint32_t i = 0; i < ShaderObject::ST_NumShaderTypes; ++ i)
		{
			if (!shader_srv_cache_[i].empty())
			{
				...
				ShaderSetShaderResources[i](d3d_11_imm_ctx_.get(), 0, static_cast<UINT>(shader_srv_ptr_cache_[i].size()), &shader_srv_ptr_cache_[i][0]);
			...
			if (!shader_sampler_cache_[i].empty())
			{
				...
				ShaderSetSamplers[i](d3d_11_imm_ctx_.get(), 0, static_cast<UINT>(shader_sampler_ptr_cache_[i].size()), &shader_sampler_ptr_cache_[i][0]);
			...
			if (!shader_cb_cache_[i].empty())
			{
				...
				ShaderSetConstantBuffers[i](d3d_11_imm_ctx_.get(), 0, static_cast<UINT>(shader_cb_ptr_cache_[i].size()), &shader_cb_ptr_cache_[i][0]);
	...
	ID3D11InputLayoutPtr const & D3D12RenderEngine::CreateD3D11InputLayout(std::vector<D3D11_INPUT_ELEMENT_DESC> const & elems, size_t signature, std::vector<uint8_t> const & vs_code)
	{
		...
		else
		{
			...
			TIF(d3d_11_device_->CreateInputLayout(&elems[0], static_cast<UINT>(elems.size()), &vs_code[0], vs_code.size(), &ia));
	...
	void D3D12RenderEngine::DoBindSOBuffers(RenderLayoutPtr const & rl)
	{
		...
		if (num_buffs > 0)
		{
			...
			for (uint32_t i = 0; i < num_buffs; ++ i)
			{
				...
				so_src[i] = d3d12_buf;
				d3d11_buffs[i] = d3d12_buf->D3DBuffer().get();
			}
			...
			d3d_11_imm_ctx_->SOSetTargets(static_cast<UINT>(num_buffs), &d3d11_buffs[0], &d3d11_buff_offsets[0]);
		...
		else
		{
			...
			d3d_11_imm_ctx_->SOSetTargets(static_cast<UINT>(num_so_buffs_), &d3d11_buffs[0], &d3d11_buff_offsets[0]);
	...
	void D3D12RenderEngine::DoRender(RenderTechnique const & tech, RenderLayout const & rl)
	{
		...
		if (all_num_vertex_stream != 0)
		{
			...
			{
				d3d_11_imm_ctx_->IASetVertexBuffers(0, all_num_vertex_stream, &vbs[0], &strides[0], &offsets[0]);
				vb_cache_ = vbs;
				vb_stride_cache_ = strides;
				vb_offset_cache_ = offsets;
			}

			D3D12RenderLayout const & d3d_rl = *checked_cast<D3D12RenderLayout const *>(&rl);
			D3D12ShaderObject const & shader = *checked_cast<D3D12ShaderObject*>(tech.Pass(0)->GetShaderObject().get());
			ID3D11InputLayoutPtr const & layout = d3d_rl.InputLayout(shader.VSSignature(), *shader.VSCode());
			if (layout != input_layout_cache_)
			{
				d3d_11_imm_ctx_->IASetInputLayout(layout.get());
		...
		else
		{
			...
			input_layout_cache_.reset();
			d3d_11_imm_ctx_->IASetInputLayout(nullptr);
		}
		...
		{
			d3d_11_imm_ctx_->IASetPrimitiveTopology(D3D12Mapping::Mapping(tt));
		...
		if (rl.UseIndices())
		{
			D3D12GraphicsBuffer& d3dib = *checked_cast<D3D12GraphicsBuffer*>(rl.GetIndexStream().get());
			if (ib_cache_ != d3dib.D3DBuffer())
			{
				d3d_11_imm_ctx_->IASetIndexBuffer(d3dib.D3DBuffer().get(), D3D12Mapping::MappingFormat(rl.IndexStreamFormat()), 0);
		...
		else
		{
			if (ib_cache_)
			{
				d3d_11_imm_ctx_->IASetIndexBuffer(nullptr, DXGI_FORMAT_R16_UINT, 0);
				ib_cache_.reset();
			}
		}
		...
		if (indirect_buff)
		{
			if (rl.UseIndices())
			{
				for (uint32_t i = 0; i < num_passes; ++ i)
				{
					...
					pass->Bind();
					d3d_11_imm_ctx_->DrawIndexedInstancedIndirect(
			...
			else
			{
				for (uint32_t i = 0; i < num_passes; ++ i)
				{
					...
					pass->Bind();
					d3d_11_imm_ctx_->DrawInstancedIndirect(
						checked_cast<D3D12GraphicsBuffer const *>(indirect_buff.get())->D3DBuffer().get(),
						rl.IndirectArgsOffset());
					pass->Unbind();
				}
			...
			if (num_instances > 1)
			{
				if (rl.UseIndices())
				{
					...
					for (uint32_t i = 0; i < num_passes; ++ i)
					{
						...
						pass->Bind();
						d3d_11_imm_ctx_->DrawIndexedInstanced(num_indices, num_instances, rl.StartIndexLocation(), rl.StartVertexLocation(), rl.StartInstanceLocation());
						pass->Unbind();
					}
				}
				else
				{
					...
					for (uint32_t i = 0; i < num_passes; ++ i)
					{
						...
						pass->Bind();
						d3d_11_imm_ctx_->DrawInstanced(num_vertices, num_instances, rl.StartVertexLocation(), rl.StartInstanceLocation());
						pass->Unbind();
					}
				}
			}
			else
			{
				if (rl.UseIndices())
				{
					...
					for (uint32_t i = 0; i < num_passes; ++ i)
					{
						...
						pass->Bind();
						d3d_11_imm_ctx_->DrawIndexed(num_indices, rl.StartIndexLocation(), rl.StartVertexLocation());
						pass->Unbind();
					}
				}
				else
				{
					...
					for (uint32_t i = 0; i < num_passes; ++ i)
					{
						...
						pass->Bind();
						d3d_11_imm_ctx_->Draw(num_vertices, rl.StartVertexLocation());
						pass->Unbind();
					}
				}
			}
	...
	void D3D12RenderEngine::DoDispatch(RenderTechnique const & tech, uint32_t tgx, uint32_t tgy, uint32_t tgz)
	{
		...
		for (uint32_t i = 0; i < num_passes; ++ i)
		{
			...
			pass->Bind();
			d3d_11_imm_ctx_->Dispatch(tgx, tgy, tgz);
			pass->Unbind();
		}
			...
			uint32_t offset)
	{
		...
		for (uint32_t i = 0; i < num_passes; ++ i)
		{
			...
			pass->Bind();
			d3d_11_imm_ctx_->DispatchIndirect(checked_cast<D3D12GraphicsBuffer*>(buff_args.get())->D3DBuffer().get(),
				offset);
			pass->Unbind();
		}
	...
	void D3D12RenderEngine::ScissorRect(uint32_t x, uint32_t y, uint32_t width, uint32_t height)
	{
		...
		d3d_11_imm_ctx_->RSSetScissorRects(1, &rc);
	}
	...
	void D3D12RenderEngine::DoDestroy()
	{
		...
		d3d_11_imm_ctx_->ClearState();
		d3d_11_imm_ctx_->Flush();
		d3d_11_imm_ctx_.reset();
	...
	void D3D12RenderEngine::DoSuspend()
	{
		...
		d3d_11_device_->QueryInterface(IID_IDXGIDevice3, reinterpret_cast<void**>(&dxgi_device));
	...
	void D3D12RenderEngine::FillRenderDeviceCaps()
	{
		...
		case D3D_FEATURE_LEVEL_10_0:
			...
			caps_.max_simultaneous_uavs = 0;
			{
				...
				d3d_11_device_->CheckFeatureSupport(D3D11_FEATURE_D3D10_X_HARDWARE_OPTIONS, &cs4_feature, sizeof(cs4_feature));
...

		{
			...
			d3d_11_device_->CheckFeatureSupport(D3D11_FEATURE_ARCHITECTURE_INFO, &arch_feature, sizeof(arch_feature));
		...
		}
		{
			...
			d3d_11_device_->CheckFeatureSupport(D3D11_FEATURE_D3D9_SIMPLE_INSTANCING_SUPPORT, &d3d11_feature, sizeof(d3d11_feature));
		...
		caps_.primitive_restart_support = (d3d_feature_level_ >= D3D_FEATURE_LEVEL_10_0);
		{
			...
			d3d_11_device_->CheckFeatureSupport(D3D11_FEATURE_THREADING, &mt_feature, sizeof(mt_feature));
		...
		caps_.shader_texture_lod_support = (d3d_feature_level_ >= D3D_FEATURE_LEVEL_10_0);
		{
			...
			d3d_11_device_->CheckFeatureSupport(D3D11_FEATURE_D3D11_OPTIONS, &d3d11_feature, sizeof(d3d11_feature));
		...
		for (size_t i = 0; i < sizeof(fmts) / sizeof(fmts[0]); ++ i)
		{
			...
			d3d_11_device_->CheckFormatSupport(fmts[i].second, &s);
			if (s != 0)
			{
				if (IsDepthFormat(fmts[i].first))
				{
					...
					UINT s1;
					d3d_11_device_->CheckFormatSupport(depth_fmt, &s1);
					if (s1 != 0)
					{
					...
					| D3D11_FORMAT_SUPPORT_DEPTH_STENCIL))
				{
					...
					{
						if (SUCCEEDED(d3d_11_device_->CheckMultisampleQualityLevels(fmts[i].second, count, &quality)))
						{
							if (quality > 0)
							{
		...
		caps_.vertex_format_support = bind<bool>(&D3D12RenderEngine::VertexFormatSupport, this,
			placeholders::_1);
		caps_.texture_format_support = bind<bool>(&D3D12RenderEngine::TextureFormatSupport, this,
			placeholders::_1);
		caps_.rendertarget_format_support = bind<bool>(&D3D12RenderEngine::RenderTargetFormatSupport, this,
		...
		case SM_DXGI:
			{
				...
				d3d_11_imm_ctx_->OMSetRenderTargets(1, &rtv, nullptr);
				...
				d3d_11_imm_ctx_->RSSetViewports(1, &vp);
		...
		case SM_NV3DVision:
			{
				...
				if (0 == eye)
				{
					...
					ID3D11Texture2DPtr stereo = checked_cast<D3D12Texture2D*>(stereo_nv_3d_vision_tex_.get())->D3DTexture();
					...
					box.front = 0;
					box.back = 1;
					d3d_11_imm_ctx_->CopySubresourceRegion(back.get(), 0, 0, 0, 0, stereo.get(), 0, &box);
				}
			}
			break;
	...
	void D3D12RenderEngine::RSSetState(ID3D11RasterizerStatePtr const & ras)
	{
		if (rasterizer_state_cache_ != ras)
		{
			d3d_11_imm_ctx_->RSSetState(ras.get());
			rasterizer_state_cache_ = ras;
		}
	}

	void D3D12RenderEngine::OMSetDepthStencilState(ID3D11DepthStencilStatePtr const & ds, uint16_t stencil_ref)
	{
		if ((depth_stencil_state_cache_ != ds) || (stencil_ref_cache_ != stencil_ref))
		{
			d3d_11_imm_ctx_->OMSetDepthStencilState(ds.get(), stencil_ref);
			depth_stencil_state_cache_ = ds;
			stencil_ref_cache_ = stencil_ref;
		}
	}

	void D3D12RenderEngine::OMSetBlendState(ID3D11BlendStatePtr const & bs, Color const & blend_factor, uint32_t sample_mask)
	{
		if ((blend_state_cache_ != bs) || (blend_factor_cache_ != blend_factor) || (sample_mask_cache_ != sample_mask))
		{
			d3d_11_imm_ctx_->OMSetBlendState(bs.get(), &blend_factor.r(), sample_mask);
	...
	void D3D12RenderEngine::VSSetShader(ID3D11VertexShaderPtr const & shader)
	{
		if (vertex_shader_cache_ != shader)
		{
			d3d_11_imm_ctx_->VSSetShader(shader.get(), nullptr, 0);
			vertex_shader_cache_ = shader;
		}
	}

	void D3D12RenderEngine::PSSetShader(ID3D11PixelShaderPtr const & shader)
	{
		if (pixel_shader_cache_ != shader)
		{
			d3d_11_imm_ctx_->PSSetShader(shader.get(), nullptr, 0);
			pixel_shader_cache_ = shader;
		}
	}

	void D3D12RenderEngine::GSSetShader(ID3D11GeometryShaderPtr const & shader)
	{
		if (geometry_shader_cache_ != shader)
		{
			d3d_11_imm_ctx_->GSSetShader(shader.get(), nullptr, 0);
			geometry_shader_cache_ = shader;
		}
	}

	void D3D12RenderEngine::CSSetShader(ID3D11ComputeShaderPtr const & shader)
	{
		if (compute_shader_cache_ != shader)
		{
			d3d_11_imm_ctx_->CSSetShader(shader.get(), nullptr, 0);
			compute_shader_cache_ = shader;
		}
	}

	void D3D12RenderEngine::HSSetShader(ID3D11HullShaderPtr const & shader)
	{
		if (hull_shader_cache_ != shader)
		{
			d3d_11_imm_ctx_->HSSetShader(shader.get(), nullptr, 0);
			hull_shader_cache_ = shader;
		}
	}

	void D3D12RenderEngine::DSSetShader(ID3D11DomainShaderPtr const & shader)
	{
		if (domain_shader_cache_ != shader)
		{
			d3d_11_imm_ctx_->DSSetShader(shader.get(), nullptr, 0);
			domain_shader_cache_ = shader;
		}
	}

	void D3D12RenderEngine::RSSetViewports(UINT NumViewports, D3D11_VIEWPORT const * pViewports)
	{
		if (NumViewports > 1)
		{
			d3d_11_imm_ctx_->RSSetViewports(NumViewports, pViewports);
		}
		else
		{
	...
	void D3D12RenderEngine::SetShaderResources(ShaderObject::ShaderType st, std::vector<tuple<void*, uint32_t, uint32_t> > const & srvsrcs, std::vector<ID3D11ShaderResourceViewPtr> const & srvs)
	{
		if (shader_srv_cache_[st] != srvs)
		{
			...
			ShaderSetShaderResources[st](d3d_11_imm_ctx_.get(), 0, static_cast<UINT>(shader_srv_ptr_cache_[st].size()), &shader_srv_ptr_cache_[st][0]);
	...
	void D3D12RenderEngine::SetSamplers(ShaderObject::ShaderType st, std::vector<ID3D11SamplerStatePtr> const & samplers)
	{
		if (shader_sampler_cache_[st] != samplers)
		{
			...
			ShaderSetSamplers[st](d3d_11_imm_ctx_.get(), 0, static_cast<UINT>(shader_sampler_ptr_cache_[st].size()), &shader_sampler_ptr_cache_[st][0]);
	...
	void D3D12RenderEngine::SetConstantBuffers(ShaderObject::ShaderType st, std::vector<ID3D11BufferPtr> const & cbs)
	{
		if (shader_cb_cache_[st] != cbs)
		{
			...
			ShaderSetConstantBuffers[st](d3d_11_imm_ctx_.get(), 0, static_cast<UINT>(shader_cb_ptr_cache_[st].size()), &shader_cb_ptr_cache_[st][0]);
	...
	void D3D12RenderEngine::DetachSRV(void* rtv_src, uint32_t rt_first_subres, uint32_t rt_num_subres)
	{
	...
	HRESULT D3D12RenderEngine::D3D11CreateDevice(IDXGIAdapter* pAdapter,
	...
	HRESULT D3D12RenderEngine::D3D12CreateDevice(IUnknown* pAdapter,
	...
	HRESULT D3D12RenderEngine::D3D11On12CreateDevice(IUnknown* pDevice, UINT Flags,
	...
	{
		return DynamicD3D11On12CreateDevice_(pDevice, Flags, pFeatureLevels, FeatureLevels,
			ppCommandQueues, NumQueues, NodeMask, ppDevice, ppImmediateContext, pChosenFeatureLevel);
	}

	HRESULT D3D12RenderEngine::D3D12GetDebugInterface(REFIID riid, void** ppvDebug) const
	{
